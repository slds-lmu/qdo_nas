...
do_bop = function(design) {
  ...
  surrogate$y_cols = c("val_loss", "latency")
  surrogate$x_cols = paste0("P", 1:30)
  ...
  ejie$domain$trafo = function(x, param_set) {
    cell = ss$get_cell(x$arch)
    path = as.integer(py_to_r(cell$encode("trunc_path")))
    as.data.table(setNames(as.list(path), paste0("P", 1:30)))
  }
...
do_random = function(design) {
  ...
  points = map_dtr(seq_len((100L - NROW(design))), function(i) data.table(arch = list(ss$get_cell()$random_cell(ss$nasbench))))
