...
def get_config_list(args):
    ...
    config_list = []
    if args.model_type == 'mobilenet_v2_torchhub':
        config_list = get_config_list_mobilenet(args)
    else:
        config_list = get_config_list_default(args)
    return config_list
...
def parse_args():
    parser = argparse.ArgumentParser(description='Example code for pruning')
    parser.add_argument('--checkpoint_name', type=str, default='checkpoint_best.pt',
                         help='checkpoint of the pretrained model')

    parser.add_argument('--model_type', type=str, default='mobilenet_v2_torchhub',
                        choices=['mobilenet_v2_torchhub', 'resnet18']) 
...
def run_pruning(args):
    ...
    experiment_dir = 'pretrained_' + args.model_type + '/'
    print(experiment_dir)

    log = open(experiment_dir + '/pruning_{}_{}_sparsity{}_{}.log'.format(
    ...
    model = create_model(model_type=args.model_type, pretrained=False, n_classes=n_classes,
                         input_size=input_size, checkpoint=experiment_dir + '/' + args.checkpoint_name)
    ...
    flops0, params0 = count_flops(model, log, device)
    ...
    if args.pruner_name in ['slim', 'taylorfo', 'mean_activation', 'apoz', 'agp']:
        ...
        if args.pruner_name == 'slim':
            kwargs['sparsifying_training_epochs'] = args.slim_sparsifying_epochs

    # pruning
    pruner = pruner_type_to_class[args.pruner_name](model, config_list, **kwargs)
    pruner.compress()
    pruner.export_model(experiment_dir + '/model_temp.pth', experiment_dir + './mask_temp.pth')
    
    # model speedup
    pruner._unwrap_model()
    if args.speed_up:
        dummy_input = torch.rand(1,3,224,224).to(device)
        ms = ModelSpeedup(model, dummy_input, experiment_dir + './mask_temp.pth')
        ms.speedup_model()
        print(model)
    ...
    val_loss, val_acc = final_loss, final_acc
    # clean up
    filePaths = [experiment_dir + '/model_tmp.pth', experiment_dir + '/mask_tmp.pth']
    for f in filePaths:
        if os.path.exists(f):
            os.remove(f)
    ...
    _, test_loss, test_acc, flops, params = test_model(model)
    del model
    if args.kd:
        del teacher_model
    return test_loss, test_acc, val_loss, val_acc, flops, params, {'flops0': flops0, 'params0': params0, 'init_loss': initial_loss, 'init_acc': initial_acc}
