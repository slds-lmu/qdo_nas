...
class EvolutionFinder:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.efficiency_n_evals = 0
        self.accuracy_n_evals = 0
    ...
    def run_evolution_search(self, verbose=False):
        ...
        self.accuracy_n_evals += len(child_pool)
...
class EvolutionFinderQDO:
    ...
    def __init__(
        self,
        constraint_type,
        efficiency_constraints,  # tuple of constraints u1, ..., uc, used for niches as [0, u1], ...
        efficiency_predictor,
        accuracy_predictor,
        **kwargs
    ):
        self.constraint_type = constraint_type
        self.efficiency_constraints = efficiency_constraints
        ...
        self.mutate_prob = kwargs.get("mutate_prob", 0.5)
        self.population_size = kwargs.get("population_size", 100)
        self.max_time_budget = kwargs.get("max_time_budget", 500)

    # sample a constraint (i.e., niche) and then an architecture belonging to that niche
    def random_sample(self):
        constraint = random.sample(self.efficiency_constraints, 1)[0]
        while True:
            sample = self.arch_manager.random_sample()
            self.efficiency_n_evals += 1
            efficiency = self.efficiency_predictor.predict_efficiency(sample)
    ...
    def mutate_sample(self, sample):
        constraint = max(self.efficiency_constraints)
        while True:
            new_sample = copy.deepcopy(sample)
    ...
    def run_evolution_search(self, verbose=False):
        ...
        max_time_budget = self.max_time_budget
        population_size = self.population_size
        mutation_numbers = population_size

        elites = [(-100, None, None)] * len(self.efficiency_constraints)  # (validation, sample, latency) tuples

        if verbose:
            print("Generate random population...")
        for _ in range(population_size):
            sample, efficiency = self.random_sample()
            self.accuracy_n_evals += 1
            acc = self.accuracy_predictor.predict_accuracy([sample]).item()
            for i in range(len(elites)):
                if efficiency <= self.efficiency_constraints[i] and acc >= elites[i][0]:
                    elites[i] = (acc, sample, efficiency)
        ...
        for iter in tqdm(
            range(max_time_budget),
            desc="Searching with %s constraints (%s)"
            % (self.constraint_type, self.efficiency_constraints),
        ):
            for _ in range(mutation_numbers):
                par_sample = random.sample(elites, 1)[0][1]
                # Mutate
                sample, efficiency = self.mutate_sample(par_sample)
                self.accuracy_n_evals += 1
                acc = self.accuracy_predictor.predict_accuracy([sample]).item()
                for i in range(len(elites)):
                    if efficiency <= self.efficiency_constraints[i] and acc >= elites[i][0]:
