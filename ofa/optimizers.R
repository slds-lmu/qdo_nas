bop_wrapper = function(job, data, instance, ...) {
  ...
  fullbudget = 1L  # FIXME:
  y_var = "val_loss"
  feature_var = as.character(instance$feature_var)
  n_enc = 128L
  nb = if (feature_var == "latency") {
    switch(as.character(instance$niches), small = ofa_note10_small_nb, medium = ofa_note10_medium_nb, large = ofa_note10_large_nb)
  } else if (feature_var == "flops") {
    switch(as.character(instance$niches), small = ofa_flops_small_nb, medium = ofa_flops_medium_nb, large = ofa_flops_large_nb)
  }

  instance = make_mobilenet_qdo(feature_var = feature_var, nb = nb, n_evals = 100L)
...
parego_wrapper = function(job, data, instance, ...) {
  ...
  instance = make_mobilenet_moo(feature_var = feature_var, n_evals = 100L)
