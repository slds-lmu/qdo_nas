...
source_files = map_chr(c("helpers_mobilenet.R", "scenarios.R", "optimizers.R"), function(x) file.path(benchmark_dir, x))
...
saveRegistry(reg)
# reg = loadRegistry("registry_qdo_nas_ofa_clean")  # to inspect the original registry on the cluster
# tab = getJobTable()
...
ofa = expand.grid(scenario = "mobilenet", instance = "imagenet", feature_var = c("latency", "flops"), niches = c("small", "medium", "large"), repls = repls)
setup = setDT(ofa)
setup[, id := seq_len(.N)]

# add problems
prob_designs = map(seq_len(nrow(setup)), function(i) {
  prob_id = paste0(setup[i, ]$scenario, "_", setup[i, ]$instance, "_", setup[i, ]$feature_var, "_", setup[i, ]$niches, collapse = "_")
  addProblem(prob_id, data = list(scenario = setup[i, ]$scenario, instance = setup[i, ]$instance, feature_var = setup[i, ]$feature_var, niches = setup[i, ]$niches, repls = setup[i, ]$repls))
...
jobs = findJobs()
resources.serial.default = list(walltime = 3600L * 18, memory = 4000, ntasks = 1L, ncpus = 1L, nodes = 1L)
...
results = reduceResultsList(done, function(x, job) {
  ...
  tmp[, feature_var := job$instance$feature_var]
...
tab = getJobTable()
as.numeric(sum(tab$time.running), units = "hours")  # 181.725 CPUh
