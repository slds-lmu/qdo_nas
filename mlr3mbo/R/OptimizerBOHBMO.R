# BOHB like MO optimizer that uses a ParEGO scalarization and EI to propse points model based
# Promotion is done via nds and hssp
#' @export
OptimizerBOHBMO = R6Class("OptimizerBOHBMO",
  inherit = Optimizer,
  public = list(
    ...
    initialize = function() {
      param_set = ps(
        ...
        acq_function = p_uty(),  # FIXME: must be AcqFunctionEI for ParEGO scalarization
        acq_optimizer = p_uty(),
        ...
        parego_s = p_int(lower = 1L),
        parego_rho = p_dbl(lower = 0, upper = 1)
      )
      param_set$values = list(rho = 0.1, acq_function = NULL, acq_optimizer = NULL, eta = 2, sampler = NULL, repeats = FALSE, parego_s = 100L, parego_rho = 0.05)
  ...
  private = list(
    .optimize = function(inst) {
      # FIXME: assert instance multicrit
      pars = self$param_set$values
      rho = pars$rho
      eta = pars$eta
      sampler = pars$sampler
      parego_s = pars$parego_s
      parego_rho = pars$parego_rho
      search_space = inst$search_space
      budget_id = search_space$ids(tags = "budget")
      k = length(inst$archive$cols_y)  # codomain can hold non targets since #08116aa02204980f87c8c08841176ae8f664980a
      ...
      lambdas = calculate_parego_weights(parego_s, d = k)
        ...
        for (s in s_max:0) {
          ...
          xdt = if (NROW(inst$archive$data) == 0L) {
            ...
            if (budget_to_fit == 0) {
              ...
              data = bo_archive$data
              ydt = data[, bo_archive$cols_y, with = FALSE]
              # FIXME: use inplace operations
              ydt = Map("*", ydt, mult_max_to_min(bo_archive$codomain))  # we always assume minimization
              ...
              mult = Map("*", ydt, lambda)
              yscal = Reduce("+", mult)
              yscal = do.call(pmax, mult) + parego_rho * yscal  # augmented Tchebycheff function
...
do_bo_proposal = function(bo_archive, acq_function, acq_optimizer, budget_id, budget_ps, n) {
  trafo = acq_function$domain$trafo  # FIXME: gets dropped below
  acq_function$surrogate$archive = bo_archive
  #acq_function$domain$subset(setdiff(acq_function$domain$ids(), budget_id))  # adjust
  #acq_function$surrogate$x_cols = setdiff(acq_function$surrogate$x_cols, budget_id)  # adjust
  acq_function$domain$trafo = trafo
  acq_function$constants = budget_ps

  acq_function$surrogate$update()
  acq_function$update()
  acq_optimizer$acq_function = acq_function
  acq_optimizer$param_set$values$n_to_propose = n
  n_patience = 100L
  i = 1L
  while (i < n_patience) {
    xdt = tryCatch(acq_optimizer$optimize(), error = function(error_condition) data.table())
    if (nrow(xdt) > 0L) {
      break
    }
    i = i + 1L
