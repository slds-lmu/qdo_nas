#' @export
OptimizerHyperbandQDO = R6Class("OptimizerHyperbandQDO",
  inherit = Optimizer,
  public = list(
        ...
        for (s in s_max:0) {
          ...
          if (s != s_max) {
            ...
            xdt_promoted = map_dtr(s_max:(s + 1), function(i) {
              ...
              row_ids = get_ni_row_ids(niches, y = y, ni = ni)
              # if we don't have enough, sample uniformly at random
              ni_add = sum(is.na(row_ids))
              if (ni_add > 0) {
                # choose the best not already selecte
                row_ids[is.na(row_ids)] = sample(y[id %nin% row_ids]$id, size = ni_add, replace = FALSE)
              }
...
# sample a niche and get the best one that has not been selected
get_ni_row_ids = function(niches, y, ni) {
  row_ids = rep(NA_integer_, ni)
  for (i in seq_len(ni)) {
    niche_sampled = sample(niches, size = 1L)
    tmp = y[niche %in% niche_sampled & id %nin% row_ids]
    row_ids[i] = if (nrow(tmp) == 0L) NA_integer_ else tmp[1L]$id
  }
  row_ids
}
