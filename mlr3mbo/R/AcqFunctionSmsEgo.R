    ...
    update = function() {
      ...
      if (is.null(self$constants$values$epsilon)) {
        ...
        epsilon = map_dbl(
          ...
          function(i) {
            (max(self$ys_front[, i]) - min(self$ys_front[, i])) / (nrow(self$ys_front) + c_val * self$progress)  # FIXME: need self$progress here! Originally self$instance$terminator$param_set$values$n_evals - archive$n_evals
          }
        )
