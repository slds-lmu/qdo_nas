...
class Cell101:
    ...
    def get_test_loss(self, nasbench, patience=50, dataset=None):
        """
        query the api until we see all three accuracies, then average them
        a few architectures only have two accuracies, so we use patience to avoid an infinite loop
        """
    ...
    def get_num_params(self, nasbench):
        # Note: num_params is unaffected by epochs
        return nasbench.query(api.ModelSpec(matrix=self.matrix, ops=self.ops))['trainable_parameters']

    def get_runtime(self, nasbench, epochs=None):
        if epochs:
            return nasbench.query(api.ModelSpec(matrix=self.matrix, ops=self.ops), epochs=epochs)['training_time']
        else:
            return nasbench.query(api.ModelSpec(matrix=self.matrix, ops=self.ops))['training_time']
